    ...
    def _match_by_tag(self, inp):
        ...
        for idx in self.joint_order:
            ...
            if self.dist_reweight:
                ...
                # min_dists = np.min(dists, axis=0, keepdims=True, where=close_tags_masks, initial=100500)
                ...
                #     poi = np.where(close_tags_masks[:, i])[0]
                #     if len(poi) > 0:
                #         ddd = dists[poi, i]
                #         d_min = np.min(ddd) + 1e-10
                #         ddd /= d_min
                #         diff_normed[poi, i] *= ddd

                # dists[np.logical_not(close_tags_masks)] = 100500
                # diff_normed[np.logical_not(close_tags_masks)] = 100500
