...
class BottomUpHigherResolutionHeadDecoupled(nn.Module):
    ...
    def __init__(self,
        ...
        #     num_deconv_filters)
        self.final_hmap_layers, self.final_tag_layers = self._make_final_layers(
            in_channels, final_layer_output_channels, extra, num_deconv_layers,
            num_deconv_filters)
    ...
    def init_weights(self):
        ...
        for _, m in self.final_hmap_layers.named_modules():
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.001, bias=0)
        for _, m in self.final_tag_layers.named_modules():
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.001, bias=0)
